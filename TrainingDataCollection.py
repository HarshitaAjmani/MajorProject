...
mode = 'trainingData'
directory = 'dataSet_honey/' + mode + '/'
minValue = 70
#Threshold value used for image processing.

#to capture video frames from the webcam (device index 0)
...
while True:
    ...
    # Data Collection
    #0xFF is used to extract the least significant 8 bits of the variable interrupt
    interrupt = cv2.waitKey(10) #this function returns ASCII code of the key pressed
    if interrupt & 0xFF == 27: 
        # esc key ASCII code is 27
        break
    if interrupt & 0xFF == ord('0'):
        cv2.imwrite(directory+'0/'+str(count['zero'])+'.jpg', test_image)#cv2.imwrite: This function is used to write an image to a file.
